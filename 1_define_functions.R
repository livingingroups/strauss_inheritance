...
pick_reproducers <- function(g, f_repro, rank.effect){
  ...
  sample(1:nrow(g), size = turnover, replace = T, 
...
pick_mortality <- function(g, f_mort, rank.efect){
  sample(1:nrow(g), size = turnover, replace = F, 
...
################################################################################
inherit_mri_youngest <- function(g, new.ids, mothers, rank.inher.precis){
  new.ids$rank <- g$rank[match(mothers$id, g$id)]+0.1
  return(new.ids)
}
################################################################################
...
################################################################################
inherit_mri_oldest <- function(g, new.ids, mothers, rank.inher.precis){
  
  for(i in 1:nrow(new.ids)){
    sibs <- g[!is.na(g$mother) & g$mother == new.ids$mother[i],]
    if(nrow(sibs)){
      ...
      new.ids$rank[i] <- max(sibs.matriline$rank)+0.1
    }else{
      new.ids$rank[i] <- g$rank[g$id == new.ids$mother[i]]+0.1
    }
  }
  return(new.ids)
}
...
################################################################################
# inherit_parent_offspring_cor <- function(g, new.ids, mothers, rank.inher.precis){
#   
#   min.change <-  -mothers$rank 
#   max.change <- max(g$rank) - mothers$rank + 1
#     
#   for(i in 1:nrow(new.ids)){
#     if(rank.inher.precis == 0){
#       new.ids$rank[i] <- runif(n = 1, min = 0, max = max(g$rank) + 1)
#     }else{
#     new.ids$rank[i] <- mothers$rank[i] + trunc_norm(mean = 0, n = 1, 
...
inherit_parent_offspring_cor <- function(g, new.ids, mothers, rank.inher.precis){
  ...
  for(i in 1:nrow(new.ids)){
    if(rank.inher.precis == 0){
      ...
      new.ids$rank[i] <- place.above - 0.1
      new.ids$rank[i] <- runif(n = 1, min = 0, max = max(g$rank) + 1)
    }else{
      new.ids$rank[i] <- mothers$rank[i] + trunc_norm(mean = 0, n = 1, 
                                                      sd = max(g$rank)/2^rank.inher.precis,
...
################################################################################
add_new_ids <- function(g, mothers, reproduce, die, f_inherit, inheritance, rank.inher.precis){
  
  if(length(reproduce)){
    reproducers <- g$id[reproduce]
    n.ids <- paste0(str_extract(reproducers,
                                '[:alpha:]+'), 
                    as.numeric(str_extract(reproducers,
                                           '[:digit:]+'))+1, 
                    '.',
                    unique(g$generation+1))
    if(any(duplicated(n.ids))){
      which.duplicated <- duplicated(n.ids, fromLast = T) | duplicated(n.ids, fromLast = F) 
      dups <- n.ids[which.duplicated]
      n.ids[which.duplicated] <- paste0(str_extract(dups,
                                  '[:alpha:]+'), 'L', strrep('i', 1:sum(which.duplicated)),
                      as.numeric(str_extract(dups,
                                             '[:digit:]+'))+1, 
                      '.',
                      unique(g$generation+1))
    }
    
    new.ids <- data.frame(id = n.ids,
                          rank = NA,
    ...
    if(any(duplicated(new.ids$id))){
      new.ids[duplicated(new.ids$id),]$id <- paste(gsub(x = new.ids[duplicated(new.ids$id),]$mother, 
                                                     pattern = '\\.', 
                                                     replacement = ''), new.ids[duplicated(new.ids$id),]$id,
                                                sep = '_')
    }
    
    new.ids <- f_inherit(g, new.ids, mothers, rank.inher.precis)
...
generation_step <- function(g, f_inherit, f_repro, f_mort, inheritance, rank.inher.precis, rank.effect){
  ...
  initial.group <- g
  reproduce <- pick_reproducers(g, f_repro, rank.effect)
  mothers <- g[reproduce,]
  die <- pick_mortality(g, f_mort, rank.effect)

  g <- add_new_ids(g, mothers, reproduce, die, f_inherit, inheritance, rank.inher.precis)
